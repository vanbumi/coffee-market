  ...
  if (products.length === 0) {
    return (
      <div className="text-center py-8">
        ...
        <p className="mt-2 text-sm text-text-secondary">Belum ada produk dari admin</p>
        <p className="text-xs text-text-tertiary">Gunakan form di atas untuk menambah produk</p>
      </div>
    );
  }
    ...
    <div>
      ...
      <div className="space-y-3">
        {filteredProducts.map((product) => (
          <div
            key={product.id}
            className="bg-surface-card border border-border rounded-lg p-4 flex items-center gap-4 hover:border-gold/20 hover:shadow-sm hover:shadow-gold/5 transition-all group"
          >
            {/* Thumbnail */}
            <div className="w-16 h-16 rounded-lg overflow-hidden bg-surface flex-shrink-0 border border-border">
              {product.images[0] ? (
            ...
            <div className="flex-1 min-w-0">
              <h4 className="font-semibold text-text-primary text-sm truncate group-hover:text-gold transition-colors">{product.name}</h4>
              <p className="text-xs text-text-secondary">{product.origin} • {product.type}</p>
