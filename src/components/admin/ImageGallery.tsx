  ...
  if (images.length === 0) {
    return (
      <div className="text-center py-8 bg-gray-50 rounded-xl border-2 border-dashed border-gray-200">
        ...
        <p className="mt-2 text-sm text-gray-500">Belum ada gambar yang diupload</p>
        <p className="text-xs text-gray-400">Upload gambar menggunakan widget di atas</p>
      </div>
    );
  }
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
        {images.map((imageUrl, index) => {
          ...
          return (
            <div
              key={index}
              className="relative group aspect-square rounded-lg overflow-hidden bg-gray-100 border border-gray-200 cursor-pointer"
